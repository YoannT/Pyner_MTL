        ...
        def fn():
            ...
            if isinstance(prep, dict):
                with fork_rng(self.data_seed):
                    data_dict = {task_name: (torch.utils.data.DataLoader(pre, shuffle=True, batch_size=self.batch_size, collate_fn=identity) if hasattr(pre, '__getitem__')
                        else torch.utils.data.DataLoader(DummyIterableDataset(pre), shuffle=False, batch_size=self.batch_size, collate_fn=identity))
                        for task_name, pre in prep.items()}
                    return data_dict
            else:
                with fork_rng(self.data_seed):
                    return (torch.utils.data.DataLoader(prep, shuffle=True, batch_size=self.batch_size, collate_fn=identity) if hasattr(prep, '__getitem__')
    ...
    def val_dataloader(self):
        def fn():
            if (getattr(self, 'val_data', None) is None):
                if not any(getattr(td, 'val_data', False) for td in self.train_data.values()):
                    return None
                else:
                    # should mean that it is multi task
                    data_dict = {}

                    with fork_rng(self.data_seed):
                        for task_name in self.train_data.keys():
                            # train_data here is not a mistake !!! (because it's multi task)
                            prep = self.preprocess(self.train_data[task_name], split="val", task_name=task_name)
                            data_dict[task_name] = (torch.utils.data.DataLoader(prep, shuffle=False, batch_size=self.batch_size, collate_fn=identity) if hasattr(prep, '__getitem__')
                                else torch.utils.data.DataLoader(DummyIterableDataset(prep), shuffle=False, batch_size=self.batch_size, collate_fn=identity))
                        return pl.trainer.supporters.CombinedLoader(data_dict, "max_size_cycle")
    ...
    def test_dataloader(self):
        ...
        def fn():
            if getattr(self, 'test_data', None) is None:
                if not any(getattr(td, 'test_data', False) for td in self.train_data.values()):
                    return None
                else:
                    # should mean that it is multi task
                    data_dict = {}
                    ...
                    with fork_rng(self.data_seed):
                        for task_name in self.train_data.keys():
                            # train_data here is not a mistake !!! (because it's multi task)
                            prep = self.preprocess(self.train_data[task_name], split="test", task_name=task_name)
                            data_dict[task_name] = (torch.utils.data.DataLoader(prep, shuffle=False, batch_size=self.batch_size, collate_fn=identity) if hasattr(prep, '__getitem__')
                                else torch.utils.data.DataLoader(DummyIterableDataset(prep), shuffle=False, batch_size=self.batch_size, collate_fn=identity))
                        return data_dict
...
def batch_to_tensors(batch, ids_mapping={}, device=None, pad=0):
    ...
    for key, rows in batch.items():
        try:
            ...
            print("Could not pad elements {} of the batch".format(key))
            raise
